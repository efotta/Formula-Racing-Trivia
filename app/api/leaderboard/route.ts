...
export async function GET(request: NextRequest) {
  try {
    ...
    const perfectRuns = await prisma.leaderboard.findMany({
      where: {
        hasPerfectRun: true,
      ...
      orderBy: [
        { perfectRunTime: 'asc' },
        { perfectRunDate: 'asc' } // Tie-breaker: earliest date wins
      ],
      take: 20,
      select: {
        userId: true,
    ...
    // Get All Levels (top 20, including all users who completed all 5 levels)
    const allLevels = await prisma.leaderboard.findMany({
      where: {
    ...
    const level4 = await prisma.leaderboard.findMany({
      where: {
    ...
    const level3 = await prisma.leaderboard.findMany({
      where: {
    ...
    const level2 = await prisma.leaderboard.findMany({
      where: {
    ...
    const level1 = await prisma.leaderboard.findMany({
      where: {
